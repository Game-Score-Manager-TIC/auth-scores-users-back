...
export class ScoresController {
    ...
    @Roles('ADMIN', 'PLAYER')
    @ApiBearerAuth()
    @ApiOperation({ summary: 'Obtener scores por usuario' })
    @ApiResponse({ status: HttpStatus.OK, description: 'Scores del usuario', type: [Score] })
    async getScoresByUserId(@Param('userId') userId: string): Promise<Score[]> {
        return await this.scoresService.getScoresByUserId(userId);
    }

    @Get('top-five')
    @ApiOperation({ summary: 'Obtener los 5 scores más altos' })
    @ApiResponse({ status: HttpStatus.OK, description: 'Top 5 scores', type: [TopScoresResponseDto] })
    async getTopFiveScores(): Promise<TopScoresResponseDto[]> {
        return await this.scoresService.getTopFiveScores();
    }
